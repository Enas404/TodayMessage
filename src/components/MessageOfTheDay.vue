...
// Function to check if the current date is a new day
const isNewDay = () => {
...
const fetchAndPlayAyah = async () => {
  ...
  if (!ayah.value) {
    await fetchAyah();
    if (ayah.value && ayah.value.audio) {
      ayahAudioRef.value.play();
    }
  }

  // Save the current date to indicate that the user has clicked the button for today
  saveCurrentDate();
};
...
// Function to save the current date in local storage
...
const checkTimeOfDay = () => {
  ...
  isMorning.value = hour >= 5 && hour < 21; // Assuming morning is from 5 AM to 12 PM
};
...
if (!clickedDate || isNewDay()) {
  isClickedToday.value = false;
} else {
  isClickedToday.value = true;
}
</script>
...
.background-container {
  ...
  width: 100%;
  height: 100%;
  overflow: auto; /* Prevent content overflow from affecting the background */
